...
| 7 | `verifySignature` setzt bei `rsassaPss` den Digest **fest auf SHA-256** und liest das Verfahren aus dem Textausdruck von `ASN1_parse_dump` statt aus den Algorithmusparametern. Die italienische Identitätskarte fällt damit durch den handgeschriebenen Prüfpfad, während OpenSSLs `CMS_verify` dieselbe Signatur bestätigt. | **ja**, trifft jedes Dokument mit PSS über einem anderen Digest |
