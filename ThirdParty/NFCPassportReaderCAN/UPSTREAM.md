...
| 7 | `verifySignature` setzt bei `rsassaPss` den Digest **fest auf SHA-256**, obwohl er bei PSS nicht im Namen des Verfahrens steht, sondern in seinen Parametern — die dieser Pfad nicht liest. Am Gerät bestätigt: die italienische Identitätskarte meldet `rsassaPss` und fällt durch den handgeschriebenen Prüfpfad, während OpenSSLs `CMS_verify` dieselbe Signatur bestätigt. | **ja**, trifft jedes Dokument mit PSS über einem anderen Digest |
...
Die Nummern 1 bis 3, 6 und 7 sind klein, allgemein richtig und ohne unseren Sonderfall
