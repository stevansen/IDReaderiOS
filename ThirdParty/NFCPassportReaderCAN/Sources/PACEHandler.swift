...
///         - Tuple of Generated Ephemeral KeyPair and the Passport's public key
    /// GEAENDERT: ein Zweig fuer klassisches DH.
    ///
    /// Das Original erzeugt in Schritt 3 bedingungslos ein `EC_KEY` aus
    /// `EVP_PKEY_get0_EC_KEY(ephemeralParams)`. Bei einem DH-Schluessel gibt das
    /// `NULL`, und heraus kommt „Failed to generate EC key" - am Geraet genau
    /// diese Meldung, sobald Schritt 2 endlich durchlief.
    ///
    /// Schritt 2 hatte schon einen DH-Zweig (`doDHMappingAgreement`), Schritt 3
    /// keinen. Der **Rest** von Schritt 3 und ganz Schritt 4 sind dagegen bereits
    /// algorithmusunabhaengig: `getPublicKeyData`, `decodePublicKeyFromBytes`,
    /// `computeSharedSecret` und `encodePublicKey` fuehren alle einen DH-Fall.
    /// Es fehlte genau diese eine Erzeugung.
    private func makeEphemeralKeyPair(from ephemeralParams: OpaquePointer) throws -> OpaquePointer {
        let keyType = EVP_PKEY_get_base_id(ephemeralParams)

        if keyType == EVP_PKEY_DH || keyType == EVP_PKEY_DHX {
            guard let params = EVP_PKEY_get1_DH(ephemeralParams) else {
                throw NFCPassportReaderError.PACEError( "Step3 KeyEx", "Keine DH-Parameter" )
            }
            defer { DH_free(params) }

            // Die **abgebildeten** Domaenenparameter uebernehmen: p und q wie
            // gehabt, aber das in Schritt 2 gerechnete neue g. Darueber dann ein
            // fluechtiges Paar erzeugen.
            guard let key = DHparams_dup(params) else {
                throw NFCPassportReaderError.PACEError( "Step3 KeyEx", "DH-Parameter nicht kopierbar" )
            }
            defer { DH_free(key) }

            guard DH_generate_key(key) == 1 else {
                throw NFCPassportReaderError.PACEError( "Step3 KeyEx", "DH-Schluesselpaar nicht erzeugbar" )
            }
            guard let pair = EVP_PKEY_new(), EVP_PKEY_set1_DH(pair, key) == 1 else {
                throw NFCPassportReaderError.PACEError( "Step3 KeyEx", "DH-Paar nicht einsetzbar" )
            }
            Logger.pace.debug( "Generated ephemeral DH key pair" )
            return pair
        }
        ...
        guard let pair = EVP_PKEY_new(), EVP_PKEY_set1_EC_KEY(pair, ephEcKey) == 1 else {
    ...
    func doStep3KeyExchange(ephemeralParams: OpaquePointer) async throws -> (OpaquePointer, OpaquePointer) {
        ...
        var ephKeyPair : OpaquePointer? = try makeEphemeralKeyPair(from: ephemeralParams)
        guard let ephemeralKeyPair = ephKeyPair else {
            throw NFCPassportReaderError.PACEError( "Step3 KeyEx", "Unable to get create ephermeral key pair" )
        }
        defer { if let offen = ephKeyPair { EVP_PKEY_free(offen) } }
